...
import { ArrowRight, Check, Flag, AlertCircle, Calendar, ShieldCheck, Trophy, ChevronLeft, Clock, Zap, Download, QrCode } from 'lucide-react';
...
export const VisitorBooking: React.FC = () => {
  ...
  // Load Experiences
  useEffect(() => {
    const exps = storageService.getExperiences().filter(e => e.isActive);
    setExperiences(exps);
    ...
    exps.forEach(exp => {
        const daySlots = bookingService.generateSlotsForDate(exp, todayStr);
  ...
  // Load Slots
  useEffect(() => {
    if (scene === 4 && selectedExpId) {
      const freshExps = storageService.getExperiences();
      const freshExp = freshExps.find(e => e.id === selectedExpId);
      if (freshExp) {
        const todayStr = format(new Date(), 'yyyy-MM-dd');
        const rawSlots = bookingService.generateSlotsForDate(freshExp, todayStr);
        const now = new Date();
  ...
  if (scene === 3) {
    ...
    return (
      <div className="max-w-6xl mx-auto p-6 animate-in fade-in duration-500">
        <button onClick={() => setScene(2)} className="mb-8 flex items-center gap-2 text-xs font-bold tracking-widest text-neutral-500 hover:text-white uppercase group">
        ...
        <div className="grid md:grid-cols-2 gap-8">
            {/* EXP 1: THE PIT LANE */}
            <div 
                onClick={() => { if (!isExp1Disabled) { setSelectedExpId(primaryExp.id); setScene(4); } }}
                className={`group relative border-2 border-white p-8 transition-all duration-300 flex flex-col min-h-[500px]
                    ${isExp1Disabled ? 'bg-neutral-900 opacity-60 cursor-not-allowed pattern-diagonal-lines' : 'bg-neutral-900 text-white hover:bg-neutral-800 cursor-pointer shadow-[12px_12px_0px_0px_#dc2626] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[16px_16px_0px_0px_#dc2626]'}
                `}
            >
                <div className="flex justify-between items-start mb-12">
                     <ShieldCheck className={`w-16 h-16 ${isExp1Disabled ? 'text-neutral-600' : 'text-white'}`} strokeWidth={1} />
                     <span className={`text-[10px] font-bold border border-current px-2 py-1 uppercase tracking-widest ${isExp1Disabled ? 'text-neutral-500' : 'text-yellow-400 border-yellow-400'}`}>
                        Multiplayer
                     </span>
                </div>
            ...
            {/* EXP 2: THE PROPHECY - UPDATED TO WHITE BACKGROUND ALWAYS */}
            <div 
                onClick={() => { if (!isExp2Disabled) { setSelectedExpId(secondaryExp.id); setScene(4); } }}
                className={`group relative border-2 border-white p-8 transition-all duration-300 flex flex-col min-h-[500px] bg-white text-black
                    ${isExp2Disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer shadow-[12px_12px_0px_0px_#facc15] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[16px_16px_0px_0px_#facc15]'}
                `}
            >
                <div className="flex justify-between items-start mb-12">
  ...
  if (scene === 4) {
    return (
      <div className="max-w-4xl mx-auto p-6 animate-in slide-in-from-right-8 duration-500">
        <button onClick={() => setScene(3)} className="mb-8 flex items-center gap-2 text-xs font-bold tracking-widest text-neutral-500 hover:text-white uppercase group">
            ...
            <div className="divide-y-2 divide-neutral-800">
                {slotsForDate.length === 0 ? (
                    ...
                    slotsForDate.map(slot => {
                        ...
                        const isDisabled = isFull || cantFit || isPassed;

                        return (
                            <div 
                                key={slot.id}
                                className={`grid grid-cols-12 items-center p-5 transition-all group ${isDisabled ? 'bg-black text-neutral-600' : 'hover:bg-neutral-800 cursor-pointer text-white'}`}
                                onClick={() => {
                                    if (!isDisabled) {
                            ...
                            >
                                <div className="col-span-3 font-mono text-xl font-bold tracking-tighter">
                                ...
                                <div className="col-span-6">
                                    {isPassed ? (
                                        ...
                                        <span className="text-[10px] font-bold uppercase tracking-widest text-orange-500">Only {slot.remainingCapacity} Seats</span>
                                    ) : (
                                        <div className="flex items-center gap-2">
  ...
  if (scene === 5 && selectedSlot) {
    return (
        <div className="max-w-3xl mx-auto p-6 animate-in slide-in-from-right-8 duration-500">
             <button onClick={() => setScene(4)} className="mb-8 flex items-center gap-2 text-xs font-bold tracking-widest text-neutral-500 hover:text-white uppercase group">
            ...
            <div className="grid md:grid-cols-3 gap-8">
                <div className="md:col-span-2 space-y-8">
                    ...
                    <div className="bg-neutral-900 border-2 border-white p-6 shadow-[8px_8px_0px_0px_#333]">
                        <h3 className="text-sm font-bold uppercase tracking-widest mb-6 border-b border-neutral-700 pb-2 text-white">Drivers ({pax})</h3>
    ...
    return (
        <div className="max-w-2xl mx-auto p-6 animate-in zoom-in-95 duration-500">
            ...
            <div className="flex items-start gap-4 mb-8 p-6 bg-neutral-900 border border-neutral-700">
                <div onClick={() => setAgreedToTerms(!agreedToTerms)} className={`mt-1 flex-shrink-0 w-6 h-6 border-2 flex items-center justify-center cursor-pointer ${agreedToTerms ? 'bg-yellow-400 border-yellow-400 text-black' : 'border-white bg-black'}`}>
                    {agreedToTerms && <Check className="w-4 h-4" />}
                </div>
                <p className="text-sm font-medium leading-relaxed cursor-pointer select-none text-neutral-300" onClick={() => setAgreedToTerms(!agreedToTerms)}>
  ...
  if (scene === 7 && confirmedBookings.length > 0) {
    ...
    return (
      <div className="min-h-[85vh] flex flex-col items-center justify-center p-6 text-center animate-in zoom-in duration-700">
        ...
        <button onClick={() => { setScene(1); setPax(1); setConfirmedBookings([]); setVisitorName(''); setAttendees([]); }} className="mt-8 text-xs font-bold uppercase tracking-widest border-b-2 border-white pb-1 text-white hover:text-yellow-400 hover:border-yellow-400 transition-colors">
